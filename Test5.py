...
# Load and resize input image to 1280x720
image_input = cv2.imread("test_image.png")
if image_input is not None:
    image_input = cv2.resize(image_input, (1280, 720))

# Verify webcam and image loaded
if not video_input.isOpened() or image_input is None:
    print("Error: Could not open webcam or load image.")
else:
    ...
    while True:
        ret, frame_webcam = video_input.read()
        if not ret:
            break
        ...
        for (x, y, w, h) in faces:
            ...
            gradient = np.linspace(0.6, 1.2, width)  # Dimming from left to right
